...
public class Automobile implements Serializable {
	...
	// new Automobile function for lap 5
	public Automobile(String make1, String model1, float basePrices, int size) {
		make = make1;
		model = model1;
	...
	// new function for lab4 Updating ************************
	public int findIndexOptSet(String name) {
		int temp = -2;
		for (int i = 0; i < optionSet.size(); i++) {
	...
	public int updateOptionName(String OptSetName, String oldOptName, String newOptName) {
		int temp = findIndexOptSet(OptSetName);
		if (temp == -2) {
			System.out.println("Find no target");
		} else {
			optionSet.get(temp).updateOptionName(oldOptName, newOptName);
		}
		return -1;
	}

	public int updateOptionPrice(String OptSetName, float oldPrice, float newPrice) {
		int temp = findIndexOptSet(OptSetName);
		if (temp == -2) {
			System.out.println("Find no target");
		} else {
			optionSet.get(temp).updateOptionPrice(oldPrice, newPrice);
		}
		return -1;
	}
	// *******************************************************************************
